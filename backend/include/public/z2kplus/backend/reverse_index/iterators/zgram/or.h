...
class Or final : public ZgramIterator {
  struct Private {};
public:
  static std::unique_ptr<ZgramIterator> create(
    std::vector<std::unique_ptr<ZgramIterator>> &&children);
  Or(Private, std::vector<std::unique_ptr<ZgramIterator>> &&children);
  ~Or() final;
  ...
  bool tryReleaseOrChildren(std::vector<std::unique_ptr<ZgramIterator>> *result) final;
